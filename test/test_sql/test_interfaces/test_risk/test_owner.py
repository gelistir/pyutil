...
class TestOwner(unittest.TestCase):
    ...
    def test_position(self):
        ...
        pdt.assert_frame_equal(o.position_by(tail=1), pd.DataFrame(index=["123"], columns=[date2str(t2), "KIID"],
                                                                   data=[[0.4, 5]]), check_dtype=False,
                               check_names=False)

        print(o.position_by(index_col="KIID"))

        pdt.assert_frame_equal(o.position_by(index_col="KIID", tail=1),
                               pd.DataFrame(index=[5], columns=pd.Index([date2str(t2)]), data=[[0.4]]),
                               check_names=False)

        pdt.assert_frame_equal(o.position_by(sum=True),
                               pd.DataFrame(index=["123", "Sum"], columns=[date2str(t1), date2str(t2), "KIID"],
    ...
    def test_volatility(self):
        ...
        pdt.assert_frame_equal(o.vola_weighted_by(index_col="KIID"),
                               pd.DataFrame(index=[5], columns=pd.Index([date2str(t1), date2str(t2)]),
                                            data=[[0.25, 1.0]]),
                               check_names=False)
    ...
    def test_kiid(self):
        ...
        pdt.assert_frame_equal(o.kiid_weighted(sum=False),
                               pd.DataFrame(index=["123"], columns=pd.Index([date2str(t1), date2str(t2)]),
                                            data=[[0.5, 2.0]]), check_names=False)
        pdt.assert_frame_equal(o.kiid_weighted(sum=True),
                               pd.DataFrame(index=["123", "Sum"], columns=pd.Index([date2str(t1), date2str(t2)]),
